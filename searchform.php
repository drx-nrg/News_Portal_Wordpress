...
<form role="search" method="get" class="search-form d-flex gap-2 align-items-center" action="<?= esc_url(home_url('/')) ?>">
    <input type="search" class="search-field form-control <?= $args['is_white'] ? 'bg-white' : '' ?> text-dark" placeholder="<?php echo esc_attr_x('Search &hellip;', 'placeholder', 'mytheme'); ?>" value="<?= get_search_query(); ?>" name="s" />
