<article id="post-<?php the_ID() ?>" <?php post_class("row w-100 d-flex align-items-center text-decoration-none text-dark mb-3 p-2 pb-3 border-bottom") ?> itemscope itemtype="https://schema.org/NewsArticle">
    <?php if (has_post_thumbnail()) : ?>
        <a href="<?php the_permalink() ?>" itemprop="url" class="col-4 d-block post-thumbnail overflow-hidden rounded-3 p-0" style="height: 10rem; box-shadow: rgba(99,99,99,.2) 0 2px 4px 0;" aria-label="Baca selengkapnya tentang <?= get_the_title() ?>" title="<?php the_title_attribute() ?>">
    ...
    <div class="col-8 mt-3 mt-md-0 px-md-4 pl-4 pl-md-0">
        ...
        <h3 style="line-height: 1.1 !important;">
            <a href="<?php the_permalink() ?>" title="<?php the_title_attribute() ?>" class="post-title text-dark text-decoration-none" itemprop="url">
                <span class="card-title mb-2 fs-4 fw-bold" itemprop="headline"><?= get_the_title() ?></span>
            </a>
        </h3>
        <div class="d-flex flex-wrap align-items-center text-secondary mb-2 fs-md-5 fw-semibold">
